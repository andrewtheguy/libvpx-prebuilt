#!/usr/bin/env bash
# Regenerate the bindings for *this platform family* from the committed libvpx headers.
#
...
# emits `::std::os::raw::c_int` and `c_char` as *aliases*, which each target resolves for
# itself, so the same source is correct on aarch64 (where c_char is unsigned) and on x86_64
# (where it is signed).
#
# **Windows is a second file, and that was measured rather than assumed.** The headers are
# libc-free, so the aliases hold there too — but bindgen also writes two things down as
# literals that differ under MSVC. `unsigned long` is 4 bytes, so `vpx_codec_cx_pkt`'s `flags`
# sits at offset 28 rather than 32 and the layout assertion for it fails to *compile* against
# the Linux file (which is the assertion doing its job). And a C enum is always `int` to MSVC,
# where clang on the other targets types an enum with no negative values `unsigned` — only an
# alias, but one `--check` would report as drift on every run. So `bindings_windows.rs` is
# generated on Windows, against the Windows SDK's `<inttypes.h>`, and lib.rs selects it by
# `cfg(windows)`. Both files come from the same eleven committed headers.
set -euo pipefail
...
# Which file this machine generates. Cross-generating with clang's `--target` is possible for
# these headers — they need only `<inttypes.h>` — but the committed file should be what the
# platform's own SDK produces, and that is also what CI checks it against.
case "$(uname -s)" in
...
generate_raw() {
  ...
  # here is an absolute path, so conversion is turned off for the call; elsewhere the variable
  # is unread. The allowlist regex accepts both path separators for the same reason: on Windows
  # clang reports the header as `include\vpx\vpx_encoder.h`, and `.*/vpx/.*` matched nothing.
  MSYS2_ARG_CONV_EXCL='*' bindgen wrapper.h \
    ...
    --raw-line "// include/vpx/ — do not edit. Regenerate with bindgen $BINDGEN_VERSION on a $platform host." \
    --raw-line "//" \
