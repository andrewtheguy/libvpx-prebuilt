...
- **No CPU floor on x86_64.** It is absent on purpose — libvpx dispatches its SIMD at run time,
  so a `-march` floor costs compatibility and buys almost nothing. A floor is worth naming for a
  codec where the flag decides whether the kernels are reachable at all; this is not one, and the
  README says why at length.
