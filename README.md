...
A commit SHA rather than the tarball checksum a `-sys` crate usually pins: libvpx publishes no
tarballs, and GitHub's generated tag archives carry no byte-stability guarantee. A commit covers
the whole tree and git verifies it.
